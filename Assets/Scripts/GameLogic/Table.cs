...
public class Table
{
    ...
    {
        for (var i = 0; i < 4; i++) // создаем строки
            _rows.Add(new List<PyramidCard>());

        for (var i = 0; i < 10; i++) // Заполняем нижнюю строку
            _rows[0].Add(new PyramidCard(deck.DrawCard()));

        for (var i = 0; i < 9; i++) // Заполняем 2-ю строку
        {
        ...
        var dependsOnId = 0;
        for (var i = 0; i < 6; i++) // Заполняем 3-ю строку
        {
        ...
        dependsOnId = 0;
        for (var i = 0; i < 3; i++) //Заполняем 4-ю строку
        {
    ...
    {
        if (!_rows[row][index].IsOpen) return false;
        _rows[row][index].IsTaken = true;
        return true;
    }

    public List<List<PyramidCard>> GetRows()
    {
        return _rows;
    }
}